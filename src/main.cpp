...
// Copyright (c) 2013-2017 The Ghostcoin developers
...
#if defined(NDEBUG)
# error "Ghostcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1b6379faa91a9f20c376748aefe522e51f139931793ef061ad208ce1087795ce");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 16); // Ghostcoin: starting difficulty is 1 / 2^16
...
{
    // Ghostcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    if (nHeight < 2)
        return 400000 * COIN; // Ghostcoin: 400K GHOST PreMine
    if (nHeight < 1442)
        return 100 * COIN; // Ghostcoin: 144K GHOST in second day
    else if (nHeight < 2882)
        return 50 * COIN; // Ghostcoin: 72K GHOST in third day
    else if (nHeight < 4322)
        return 25 * COIN; // Ghostcoin: 36K GHOST in third day

    int64 nSubsidy = 2 * COIN; // Ghostcoin: 2.88K GHOST per day thereafter

    // Ghostcoin: 13M total GHOST
    if (nHeight > 6178322)
        nSubsidy=0;
...
static const int64 nTargetTimespan = 12 * 60 * 60; // Ghostcoin: 12 hours
static const int64 nTargetSpacing = 60; // Ghostcoin: 60 seconds
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespan2Q13 = 60 * 60; // Ghostcoin: 60 minutes
...
unsigned int static GetNextWorkRequiredGhostcoin(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ghostcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    ...
    int64_t PastBlocksMin = 60; // Ghostcoin: look-back 60 minutes
    int64_t PastBlocksMax = 60; // Ghostcoin: look-back 60 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    } else if (retarget == DIFF_GHOST) {
        // Retarget using Ghostcoin style retargeting
        return GetNextWorkRequiredGhostcoin(pindexLast, pblock);
    } else  {
        // Retarget using Classic Ghostcoin style retargeting
        unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
...
void ThreadScriptCheck() {
    RenameThread("ghostcoin-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ghostcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1b6379faa91a9f20c376748aefe522e51f139931793ef061ad208ce1087795ce");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("12345604078184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515895207;
        ...
        assert(block.hashMerkleRoot == uint256("0xfdd51d8c61454098a61cb0ba59d91c52c55df4c6d9555f65f6eb7e11731fadb9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x63, 0x42, 0x21, 0x2c }; // Ghostcoin: switched at 1393657079.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GhostcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GhostcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GhostcoinMiner(CWallet *pwallet)
{
    printf("GhostcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running GhostcoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GhostcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GhostcoinMiner, pwallet));
}
